// dynamic solution: O(N) 하나의 문제는 단 한번만 푼다

// 문자열 자기 자신은 그 자체로 prefix이자 suffix인데, 이는 고려 대상에서 제외한다.
const LPS = function (str) {
    // lps[i]는 0부터 i까지의 부분 문자열, 즉 str.slice(0, i + 1)에서 'lps의 길이'를 저장한다.
    const lps = Array(str.length);//->초기에는 비어있지만 str.length만큼 공간이 할당된 배열
  //인덱스 === rightIdx
  //요소 === leftIdx(매칭된 개수만큼 늘어나서 저장됨)
  //즉 같은 prefix와 sufix가 같은 것은 짝을 지어주는 것을 나타냄
  
    // lps[0]은 길이가 1인 문자열의 lps의 길이이므로 항상 0이다. (자기 자신 포함 금지)
    lps[0] = 0;
    
    let leftIdx = 0; //초반에는 가장 왼쪽 인덱스(0)를 의미하지만, 매칭될 때마다 카운트를 올리면서 lps의 길이를 나타내기도 함
    let rightIdx = 1;
    // lps[i]를 1부터, 즉 문자열의 길이가 2일때부터 차례대로 구한다.
    while (rightIdx < str.length) {
  
      //leftIdx를 기준으로 rightIdx를 늘려가며 순회
      //중간에 매칭이 된 경우                    전체 문자열 중에서 suffix는 절반이상의 인덱스에 위치
      if (str[leftIdx] === str[rightIdx] && rightIdx >= str.length / 2) {
        // 가장 단순한 경우를 생각해보면, 쉽게 이해할 수 있다.
        // 1) 길이가 2 경우
        // 2) 길이가 3 이상인데 전부 같은 문자인 경우
        // 0부터 leftIdx까지 매칭이 되었으므로 매칭된 길이는 leftIdx + 1이다.
        leftIdx++;
        lps[rightIdx] = leftIdx;
        rightIdx++;//문자열 끝까지 탐색하겠다
      } 
      else {
        // 중간에 매칭이 되지 않은 경우, leftIdx를 조정한다.
  
        // 현재 lps[0]부터 lps[rightIdx - 1]까지 계산이 완료된 상태이다.
        // 처음부터 다시 prefix, suffix 매칭을 하는 것이 원칙이지만
        // 앞서 계산한 결과인 lps 배열을 통해 처음으로 되돌아갈 필요는 없다. -> dynamic programming
  
        // 예. aaabaaaa
        // 현재 leftIdx는 2, rigthIdx는 3, lps는 [0, 1, 2]인 상태라고 가정해보자.
        // leftIdx가 1일 때까지, 즉 현재 leftIdx 직전(leftIdx - 1)까지는 매칭이 되었다.
        //rightIdx를 기준으로 leftIdx를 이전 요소로 넘기면서 매칭되는 거 있는지 확인
        if (leftIdx !== 0) {
          leftIdx = lps[leftIdx - 1];
    
        } else {
          // rightIdx가 1인 경우, 즉 첫 iteration일 경우
          // lps[rightIdx]가 0인 것은 명백하다. (예. ab)
          // leftIdx가 0이라는 것은 처음부터 prefix, suffix 매칭을 하는 경우이다.
          // lps가 존재하지 않는 경우이다.
          lps[rightIdx] = 0; //짝 지을 것이 없다
          rightIdx++;//문자열 끝까지 탐색하겠다
        }
      }
    }
    const res = lps[lps.length - 1];
    //lps의 마지막 요소 
    //=== 매칭된 만큼의 길이
    //                          ex.'aaaa'
    return res > str.length / 2 ? Math.floor(str.length / 2) : res;
  };